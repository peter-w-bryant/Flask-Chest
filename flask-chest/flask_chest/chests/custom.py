...
class FlaskChestCustomWriter(FlaskChest):
    ...
    def write(
        ...
        if response.status_code in self.success_status_codes:
            self.logger.debug(
                f"FlaskChestCustomWriter: Success! Status code {response.status_code} received from custom writer!"
            )
        else:
            raise Exception(
                f"FlaskChestCustomWriter: Failure! Status code {response.status_code} not in success status codes {self.success_status_codes}!"
            )
